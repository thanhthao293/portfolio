...
```{r fig1, fig.cap = "Figure 1. Distribution of cell morphology counts by treatment (black points represent the average count for each treatment", fig.height=4, fig.width=8}
...
```{r, fig.cap = "Figure 2. Distribution of hyphae-forming cells by treatment replicate"}
...
```{r}
# fit a poisson glm with offset
offset_pois_glm <- glm(GT_total ~ treatment, 
                       family = poisson(link=log), data = plant, offset = rep(200, nrow(plant)))
...
emm <- emmeans(offset_pois_glm, "treatment", type="response")
```
...
summary(h1_test) %>% 
  ...
  cols_align(align = "center", columns = c(ratio, p.value)) %>%
  cols_label(contrast = "Comparison",
             p.value = "P-value") %>%
  cols_width(vars(contrast) ~ px(275),
             vars(ratio) ~ px(100),
             vars(p.value) ~ px(100)) %>%
