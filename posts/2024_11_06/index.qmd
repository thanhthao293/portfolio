...
```{r}
# read in data
plant <- read.csv("C:/Users/thaon/OneDrive/Miami University/Fall 2024/STA 660/[Project 4] Plant Origin Compounds Effect on Fungal Morphogenesis/LeeDataFungalMorphogenesis.csv")
...
$$
log(\frac{X_{ij}}{200}) = \beta_0 + \beta_1 \times I(\text{trt}_1 = \text{Whole}) + \beta_2 \times I(\text{trt}_2 = \text{A}) + \ldots + \beta_7 \times I(\text{trt}_7 = \text{A-SA and B-SA}) \,+\,log(200)
$$
...
ggplot(plant, aes(x=treatment, y=GT_total, col=treatment)) +
  ...
  labs(caption = "Figure 1. Distribution of cell morphology counts by treatment (black poits represent the average count for each treatment)",
       x = " ",
       y = "Count") +
  theme_bw() +
...
The distribution of cells forming hyphae is examined in figure 2. It shows a right-skewed pattern. Consequently, tests assuming normality, such as one-way ANOVA, may not be suitable. Therefore, alternative methods for modeling the treatment effect on the total number of cells forming hyphae should be explored.

```{r}
ggplot(plant, aes(x=GT_total)) +
  geom_histogram(aes(y = ..density..),  binwidth=10, fill = "#3a90b3") +
  geom_density(size=0.5) +
  labs(caption = "Figure 2. Distribution of hyphae-forming cells by treatment replicate",
       x = "Count",
       y = "Density") +
  theme_minimal() +
...
cbind(
  ...
  cols_width(vars(contrast) ~ px(300),
             vars(ratio, SE, p.value, CI) ~ px(100)) %>%
